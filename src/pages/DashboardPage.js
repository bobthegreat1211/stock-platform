...
export default function DashboardPage({ portfolio }) {
  ...
  useEffect(() => {
    async function loadIndexes() {
      const fetchIndex = async (symbol) => {
        const res = await fetch(
          `https://backend-r60u3sswg-bobthegreat1211s-projects.vercel.app/api/index?symbol=${symbol}`
        );
    ...
    async function loadTrending() {
      const res = await fetch(
        "https://backend-r60u3sswg-bobthegreat1211s-projects.vercel.app/api/scan?mode=balanced"
      );
  ...
  useEffect(() => {
    ...
    async function loadHistory() {
      try {
        ...
        const res = await fetch(
          `https://backend-r60u3sswg-bobthegreat1211s-projects.vercel.app/api/history?ticker=${popup.symbol}`
        );
        const data = await res.json();
